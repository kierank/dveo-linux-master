/* dvbm_q3inoe.c
 *
 * Linux driver functions for Linear Systems Ltd. DVB Master Quad-3in1out.
 *
 ...
 * @pdev: PCI device
 *
 * Handle the insertion of a DVB Master Quad-3in1out.
 * Returns a negative error code on failure and 0 on success.
 **/

int __init
dvbm_q3ino_pci_probe (struct pci_dev *pdev)
{
	int err, i;
	unsigned int cap;
	...
	/* Initialize the driver_data pointer so that dvbm_q3ino_pci_remove()
...
/**
 * dvbm_q3ino_irq_handler - DVB Master Quad-3in1out interrupt service routine
 * @irq: interrupt number
 * @dev_id: pointer to the device data structure
 * @regs: processor context
 **/
static irqreturn_t
IRQ_HANDLER (dvbm_q3ino_irq_handler, irq, dev_id, regs)
{
